  ...
  return (
    ...
    >
      {[...Array(6).keys()].map((i) => {
        return (
          <div
            key={`work-grid-skeleton-${i}`}
            className={clsx(
              "mb-4 w-full animate-pulse rounded-sm bg-gray-100 backdrop-blur-sm",
              i % 3 == 0 ? "opacity-80" : "opacity-60",
              i % 2 == 0 && "animation-delay-3500"
            )}
            style={{ height: 350 + Math.random() * 100 }}
          />
        );
      })}
    </Masonry>
